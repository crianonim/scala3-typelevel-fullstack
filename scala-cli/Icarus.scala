...
def consumableLinks = {
  ...
  val consumableLinks = doc >> "#mw-content-text > div.category-page__members" >> elementList(
    "li > a "
  ).map(a => a >> attr("href"))
  consumableLinks.map(link => {
    println(
      s"curl https://icarus.fandom.com$link -o ${link.drop(1).filter(x => x.isLetter || x == '/')}.html"
    )
  })
}
